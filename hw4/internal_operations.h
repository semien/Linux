...
int print_file_from_block(u64 data_block_offset, int deep){
  ...
  if (deep == 1){
    //printf("%s\n", data_block);
    bzero(internal_mes.data, MFS_BLOCK_SIZE);
    memcpy(internal_mes.data, data_block, MFS_BLOCK_SIZE);
    send(sock, &internal_mes, sizeof(struct message), 0);
  } else {
    u64* blocks = (u64*)data_block;
    for (i = 0; i < MFS_BLOCKS_PER_BLOCK; ++i){
      if (blocks[i] == 0)
